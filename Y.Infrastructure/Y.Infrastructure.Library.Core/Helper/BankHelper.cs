...
namespace Y.Infrastructure.Library.Core.Helper
{
    ...
    {
        private static readonly string BankCode =
        ...
        /// 银行缩写获取银行中文名称
        /// </summary>
        public static Dictionary<string, string> BankCodeDic =>
            JsonConvert.DeserializeObject<Dictionary<string, string>>(BankCode);
        ...
        /// <returns></returns>
        public static bool ValidateCardByAlipayInterface(string cardNo)
        {
            // 储蓄卡（DC）还是信用卡（CC）
        ...
        /// 验证银行卡号是否符合Luhn规则
        /// </summary>
        /// <param name="no">银行卡号码</param>
        /// <returns></returns>
        public static bool ValidateCardNoByLuhn(string no)
        {
            //第一步：从右边第1个数字开始每隔一位乘以2；
            //第二步： 把在第一步中获得的乘积的各位数字相加，然后再与原号码中未乘2的各位数字相加；
            //第三步：对于第二步求和值中个位数求10的补数，如果个位数为0则该校验码为0。
            System.Text.RegularExpressions.Regex regex = new System.Text.RegularExpressions.Regex(@"^\d{7,21}$");
