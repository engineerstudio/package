...
namespace Y.Infrastructure.Library.Core.Extensions
{
    public static class StringExtensions
    {
        ...
        public static Dictionary<string, string> JsonToDictionary(this string json)
        {
            ...
            foreach (var j in jo)
                dic.Add(j.Key, j.Value.To<string>());
            return dic;
        }
        ...
        /// <param name="json"></param>
        /// <returns></returns>
        public static T JsonStringToEntity<T>(this string json) where T : class
        {
        ...
        {
            System.Reflection.PropertyInfo[] properties = str.GetType()
                .GetProperties(System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public);
            foreach (PropertyInfo p in properties)
            {
                object[] attr = p.GetCustomAttributes(typeof(DescriptionAttribute), false);
            }
            //str.GetCustomAttributes(typeof(DescriptionAttribute), false);
            //List<PropertyInfo> properties = str.GetType().GetProperties(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.DeclaredOnly ).ToList();
            //Func<PropertyInfo, bool> filter = a => ((DescriptionAttribute)a.GetCustomAttributes(typeof(DescriptionAttribute), true).FirstOrDefault()).Description == "Description";
            //PropertyInfo property = properties.Where(filter).FirstOrDefault();
            //string value = property.GetValue(str, null).To<String>();//取值

            return str.GetType().GetProperty("DescriptionAttribute").GetValue(str, null).To<String>();
        }
    }
}